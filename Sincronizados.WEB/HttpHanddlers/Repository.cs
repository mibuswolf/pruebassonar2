...
namespace Sincronizados.WEB.HttpHanddlers
{
    public class Repository : IRepository
    {
        ...
        public async Task<HttpResponseWrapper<object>> PostPDF<T>(string url, T model)
        {
            var mesageJSON = JsonConvert.SerializeObject(model);
        ...
        {
            var mesageJSON = System.Text.Json.JsonSerializer.Serialize(model);
            var messageContet = new StringContent(mesageJSON, Encoding.UTF8, "application/json");
            var responseHttp = await _httpClient.PostAsync(url, messageContet);
            return new HttpResponseWrapper<object>(null, !responseHttp.IsSuccessStatusCode, responseHttp);
        }
        ...
        public async Task<HttpResponseWrapper<TResponse>> Post<T, TResponse>(string url, T model)
        {
            ...
            if (responseHttp.IsSuccessStatusCode)
            {
                //  var response  = Newtonsoft.Json.JsonConvert.DeserializeObject<TResponse>(responseHttp.Content.ReadAsStringAsync().ToString());
        ...
        private async Task<T> UnserializeAnswer<T>(HttpResponseMessage httpResponse, JsonSerializerOptions jsonSerializerOptions)
        {
            ...
            return System.Text.Json.JsonSerializer.Deserialize<T>(respuestaString, jsonSerializerOptions)!;
        }
